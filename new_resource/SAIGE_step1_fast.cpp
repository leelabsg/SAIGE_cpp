...
void init_global_geno(const std::string& bed, const std::string& bim, const std::string& fam,
                      ...
                      bool setKinDiagtoOne, double minMAFforGRM) {
  geno.setGenoObj(bed, bim, fam, subSampleInGeno, indicatorGenoSamplesWithPheno, setKinDiagtoOne, minMAFforGRM);
}
                               ...
                               double min_maf, double max_miss)
{
  ...
  arma::ivec sub; sub.set_size(std::count(keep.begin(), keep.end(), true));
  { int k=0; for (int i=0;i<(int)keep.size();++i) if (keep[i]) sub[k++] = i; }
  ...
  // accumulate kinship across markers for those pairs
  arma::fvec kin(npairs, arma::fill::zeros);
  parallelcalsparseGRM(kin);

  // assemble COO + add diagonal = 1.0
  const int n  = geno.getNnomissing();
  const int nz = npairs + n;
...
// Generate ±1 (Rademacher) vector of length n without Rcpp/R
static inline arma::fvec rademacher_vec(int n) {
  arma::fvec u = arma::randu<arma::fvec>(n);          // [0,1)
  u.transform([](float z){ return (z < 0.5f) ? -1.0f : 1.0f; });
  return u;
}
               ...
               float traceCVcutoff)
{
  ...
  while (traceCV > traceCVcutoff) {
    ...
    const double sd = std::sqrt(arma::var(slice));        // unbiased
    traceCV = (mu != 0.0) ? static_cast<float>(sd / std::abs(mu))
...
arma::vec GetLambda0(const arma::vec& lin_pred, const Rcpp::List& inC) {
    Rcpp::DataFrame timedata = inC["timedata"];
